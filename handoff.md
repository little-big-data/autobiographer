...
**Test Files**:
`packages/localizer/tests/test_flickr_plugin.py` (new — 28 test functions, 36 collected
test cases after a 9-way parametrization). Confirmed RED: `cd packages/localizer &&
python -m pytest tests/test_flickr_plugin.py -v` → **0 passed, 36 failed**, all either
`ModuleNotFoundError: No module named 'localizer.plugins.flickr'` or the registration
`AssertionError` — no unrelated collection errors, no vacuous passes.
...
**Test Files**:
`packages/localizer/tests/test_untappd_plugin.py` (new — 22 tests). Confirmed RED:
`cd packages/localizer && python -m pytest tests/test_untappd_plugin.py -v` → **0
passed, 22 failed**, each either `ModuleNotFoundError: No module named
'localizer.plugins.untappd'`, an `AssertionError` on the `REGISTRY` membership check, or
(for the no-network-imports test) `FileNotFoundError` on the not-yet-created
`loader.py`.
...
`test_untappd_loader_has_no_network_imports`.
...
`brewery_name`/`beer_name`/`beer_type` per the plan's explicit mapping. No
`LocalizerStore`/DuckDB involved — `tmp_path`-based CSV fixtures only.
